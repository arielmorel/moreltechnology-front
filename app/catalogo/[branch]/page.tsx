...
export default function CatalogoBranchPage({ params }: { params: Promise<{ branch: string }> }) {
  ...
  useEffect(() => {
    params.then(p => {
      const branchId = p.branch;
      if (VALID_BRANCHES.includes(branchId)) {
        setBranch(branchId);
      } else {
        setBranch(DEFAULT_BRANCH);
        router.replace(`/catalogo/${DEFAULT_BRANCH}`);
      }
      setBranchReady(true);
    });
  }, [params, router]);
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      try {
        if (debouncedSearch.trim() === "") {
          const { products: allProducts } = await getProducts(0, PRODUCTS_PAGE_SIZE, selectedCategory, branch);
          setProducts(allProducts);
        } else {
          const { products: searchResults } = await searchProducts(debouncedSearch, 0, SEARCH_PAGE_SIZE, selectedCategory, branch);
    ...
    loadProducts();
  }, [debouncedSearch, selectedCategory, branch, branchReady]);
