...
import { ArrowRight, BadgePercent, Star, Sparkles } from "lucide-react";
...
export type ProductCarouselType = "offers" | "featured" | "related";
  ...
  return (
    <section className="py-16 md:py-20 bg-muted/30 relative overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] ${type === "offers" ? "bg-red-500/5" : "bg-primary/5"} blur-[120px] rounded-full`} />
      </div>

      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
          <div className="space-y-2">
            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${type === "offers" ? "bg-red-500/10 border-red-500/20" : "bg-primary/10 border-primary/20"} ${config.accentColor} text-xs font-bold uppercase tracking-widest border`}>
              <Icon className={`w-3.5 h-3.5 ${config.iconColor}`} />
              {config.title}
            </div>
            <h2 className="text-3xl md:text-4xl font-black tracking-tight">
              {type === "offers" ? <>Ofertas <span className="text-red-600">Flash</span></> : config.title}
            </h2>
