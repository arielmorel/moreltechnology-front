...
export function ProductCard({ product, variant = "default" }: ProductCardProps) {
  ...
  return (
    ...
    >
      <Card className={cn(
        "h-full flex flex-col overflow-hidden border border-border/50 bg-card/50 py-0 backdrop-blur-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 group",
        ...
        <Link href={productUrl(product.slug)} className="flex-1 flex flex-col">
          <CardHeader className="p-0 relative">
            <div className={cn(
              ...
              {!isOutOfStock && variant === "compact" && product.originalPrice && (
          ...
          <CardContent className={variant === "compact" ? "flex-1 p-4" : "flex-1 p-5"}>
            <div className="mb-2">
              ...
              <h3 className={cn(
                variant === "compact" ? "min-w-0 font-semibold text-sm line-clamp-2 group-hover:text-red-600 transition-colors mb-3 min-h-[2.5rem]" : "min-w-0 font-semibold text-sm sm:text-lg line-clamp-2 mt-1 group-hover:text-primary transition-colors",
                isOutOfStock && "text-muted-foreground"
              )}>{product.name} | {product.description}</h3>
            </div>
            {variant === "compact" ? (
              ...
              <>
                <div className="hidden sm:flex items-center gap-2 mb-4">
                  <div className={cn(
                    ...
                    isOutOfStock ? "bg-red-500" : product.quantity > 5 ? "bg-green-500 animate-pulse" : "bg-orange-500 animate-pulse"
                  )} />
                  <span className={cn(
                    "text-[10px] font-bold uppercase tracking-widest",
                    isOutOfStock ? "text-red-600" : product.quantity > 5 ? "text-green-600" : "text-orange-600"
                  )}>
                    {isOutOfStock ? "Agotado" : product.quantity > 5 ? "Stock Disponible" : `Solo ${product.quantity} disponibles`}
                  </span>
                </div>
        ...
        {variant === "default" && (
          <CardFooter className="p-3 pt-0 sm:p-5 sm:pt-0 flex flex-col gap-3 sm:gap-4">
            {isOutOfStock ? (
              <>
